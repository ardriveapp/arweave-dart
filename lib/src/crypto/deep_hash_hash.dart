...
Future<Uint8List> deepHashHash(dynamic data) async {
  if (data is Stream<List<int>>) {
    ...
    final taggedHash = sha384Hash(tag) + sink.digest();
...
Future<Uint8List> deepHashChunks(dynamic chunks, Uint8List acc) async {
  ...
  final hashPair = acc + await deepHashHash(chunks[0]);
...
Uint8List sha384Hash(List<int> data) {
  return SHA384().update(data).digest();
}
